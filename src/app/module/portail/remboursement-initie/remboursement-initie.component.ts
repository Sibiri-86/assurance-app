...
@Component({
  selector: 'app-remboursement-initie',
  templateUrl: './remboursement-initie.component.html',
  styleUrls: ['./remboursement-initie.component.scss']
})
export class RemboursementInitieComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.remboursementList$.pipe(takeUntil(this.destroy$)).subscribe((value) => {
      ...
      if (value) {
        ...
        this.fileTree = [];
        for(let i= 0 ; i< this.remboursementList.length; i++) {
          this.fiTree.label = this.remboursementList[i].dateSaisie.toString();
          this.fiTree.icon = "pi pi-book"
          this.fiTree.key = i.toString();
          if(this.remboursementList[i].documents && this.remboursementList[i].documents.length>0) {
            for(let j= 0 ; j< this.remboursementList[i].documents.length; j++) {
              this.children.label = this.remboursementList[i].documents[j].nom;
              this.children.icon = "pi pi-file"
              this.fiTree.key = j.toString();
              this.children.data = this.remboursementList[i].documents[j].url;
...
verifieDateSoins(event){
 ...
 //   this.store.dispatch(featureActionPrefinancement.createPrefinancement({prefinancement: this.prefinancementList}));
  ...
  onCreate() {
    ...
    this.remboursement = this.prestationForm.value;
