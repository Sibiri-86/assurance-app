...
$getPharmacieGardes(): Observable<PharmacieGardeList> {
    // @FIXME: get request
    return this.http.get( `${GlobalConfig.getEndpoint(Endpoints.PARAMETRAGE_PHARMACIE_GARDE)}`).pipe(
      map((response: PharmacieGardeList) => response),
...
pushFileToStorage(file: File): Observable<any> {
  ...
  return this.http.post(`${GlobalConfig.getEndpoint(Endpoints.PARAMETRAGE_PHARMACIE_GARDE)}/upload`, data, { headers: headers });
}
